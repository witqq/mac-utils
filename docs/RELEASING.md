...
Open **Actions → App Store Metadata → Run workflow** on `main`, enter the already uploaded version and build, and choose `sync`. Approve the protected `app-store` deployment. The workflow uses Fastlane 2.238.0 under Ruby 3.4.10 for product-page and submission operations, plus Apple’s current availability v2 and price-schedule APIs for the free worldwide distribution contract. It synchronizes both localizations, all eight screenshots, copyright, Utilities/Productivity categories, the age-rating declaration, review contact and notes, automatic release after approval, and export/content-rights declarations. It selects the build only when `submit` is requested.
