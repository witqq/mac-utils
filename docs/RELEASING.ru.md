...
Откройте **Actions → App Store Metadata → Run workflow** на `main`, укажите уже загруженные version и build и выберите `sync`. Подтвердите защищённый deployment `app-store`. Workflow использует Fastlane 2.238.0 и Ruby 3.4.10 для product-page и submission operations, а актуальные Apple availability v2 и price-schedule APIs — для контракта бесплатного распространения во всех регионах. Он синхронизирует две локализации, все восемь screenshots, copyright, категории Utilities/Productivity, age-rating declaration, контакт и заметки для review, автоматический выпуск после одобрения и declarations об encryption/content rights. Build выбирается только в режиме `submit`.
